...
function renderBottles(data) {
  ...
  Object.keys(data).forEach((key) => {
    ...
    if (item && typeof item.uid !== "undefined" && typeof item.weight !== "undefined") {
      ...
      const displayName =
        item.name && item.name.trim() !== "" ? `<strong>${item.name}</strong>` : `<strong>${key}번 소스통</strong>`;
...
function renderSources(data) {
  ...
  Object.keys(kindGroups).forEach((kind) => {
    ...
    const bottleNames = group
      .map(({ item }) => (item.name && item.name.trim() !== "" ? item.name : "(이름 없음)"))
      .join(", ");
...
function renderSourceCard(key, item, isMerged) {
  ...
  if (isMerged) {
    ...
    const kindName =
      item.kind !== undefined && kindNameMap[item.kind] ? kindNameMap[item.kind] : `종류 ${item.kind ?? "-"}`;
    const displayName = item.name && item.name.trim() !== "" ? `${item.name} / ${kindName}` : `${key} / ${kindName}`;
...
function deleteSource() {
  ...
  const keyStr = String(currentEditKey);

  const confirmDelete = confirm(`정말로 UID "${keyStr}"의 소스통을 삭제하시겠습니까?`);

  if (confirmDelete) {
    // 1. 먼저 소스통의 uid 값을 가져온다
    database
      .ref(`/${keyStr}`)
      .once("value")
      .then((snapshot) => {
        ...
        if (!data || typeof data.uid === "undefined") {
          throw new Error("소스통 데이터 또는 UID를 찾을 수 없습니다.");
        }

        const uid = String(data.uid); // 실제 uid 값 사용

        // 2. 삭제 업데이트 구성
        const updates = {};
        updates[`/${keyStr}`] = null; // 소스통 삭제
        updates[`/uids/${uid}`] = null; // uids 안의 해당 uid 삭제
...
document.getElementById("deleteKindBtn").onclick = function () {
  ...
  const confirmDelete = confirm(
    `정말로 "${kindNameMap[selectedKind]}" (${selectedKind}) 소스 종류를 삭제하시겠습니까?\n※ 이 종류를 사용 중인 소스들은 표시되지 않을 수 있습니다.`
  );
